    ...
    def ppo(args):
        ...
        for i_episode in range(args.num_episode):
            ...
            for i in reversed(range(batch_size)):
                ...
                # ref: https://arxiv.org/pdf/1506.02438.pdf (generalization advantage estimate)
                advantages_novel[i] = deltas_novel[i] + args.gamma * args.lamda * prev_advantage_novel * masks_novel[i]
            ...
            if args.advantage_norm:
                ...
                '''Start Choreo'''
                advantages_novel = (advantages_novel - advantages_novel.mean()) / (advantages_novel.std() + EPS)
                '''End Choreo'''
            ...
            for i_epoch in range(int(args.num_epoch * batch_size / args.minibatch_size)):
                ...
                if args.lossvalue_norm:
                    minibatch_return_6std_novel = 6 * minibatch_returns_novel.std() + EPS
                    loss_value_novel = torch.mean((minibatch_newvalues_novel - minibatch_returns_novel).pow(2)) / minibatch_return_6std_novel
                else:
                    loss_value_novel = torch.mean((minibatch_newvalues_novel - minibatch_returns_novel).pow(2))

                loss_entropy_novel = torch.mean(torch.exp(minibatch_newlogproba_novel) * minibatch_newlogproba_novel)

                total_loss_novel = loss_surr_novel + args.loss_coeff_value * loss_value_novel + args.loss_coeff_entropy * loss_entropy_novel
                '''End Choreo'''


                # Update alpha based on Hong et al.
                if reward_novel_sum >= Lower_Novel_Bound:
                    alpha = 0.99 * alpha
                else:
                    alpha = 1.01 * alpha

                total_loss = total_loss + alpha * total_loss_novel
